...
- Works on Linux/macOS/Windows (Docker Desktop) as long as Docker is available.
...
from typing import List, Optional, Sequence
    ...
    def _require_docker() -> None:
        if not _has_docker():
            raise SystemExit("Docker is required but not available on PATH or not running.")
    ...
    def _file_arg(flag: str, path: Optional[Path]) -> List[str]:
        if not path:
            return []
    ...
    def _workflow_args(path: Optional[str]) -> List[str]:
        if not path:
            return []
        return ["-W", path]


    def _job_args(job: Optional[str]) -> List[str]:
        if not job:
            return []
        return ["-j", job]
    ...
    def _parse_platform(values: Optional[List[str]]) -> List[str]:
        if not values:
            return []
        out: List[str] = []
        for v in values:
    ...
    def parse_args(argv: Optional[Sequence[str]] = None) -> ActRunSpec:
        p = argparse.ArgumentParser(
            description="Run GitHub Actions workflows locally (repo-root) using Docker + act."
        )
        p.add_argument(
            "--repo-root",
            required=True,
        ...
        p.add_argument(
            "--workflow",
            help="Optional workflow path or directory passed to act (-W). "
        ...
        p.add_argument(
            "--platform",
            action="append",
            help='Platform mapping(s) (-P). '
            'Example: --platform "ubuntu-latest=ghcr.io/catthehacker/ubuntu:act-latest"',
        )
        p.add_argument("--secrets-file", type=Path, help="Path to act secrets file.")
        p.add_argument("--env-file", type=Path, help="Path to act env file.")
        p.add_argument(
            "--act-image",
            default=DEFAULT_ACT_IMAGE,
            help=f"act Docker image (default: {DEFAULT_ACT_IMAGE}).",
        )
        p.add_argument(
            "--bind-workdir",
            default=DEFAULT_CONTAINER_WORKDIR,
            help=f"Container path to mount repo at (default: {DEFAULT_CONTAINER_WORKDIR}).",
        )
        p.add_argument(
            "--reuse",
            action="store_true",
        ...
        p.add_argument(
            "--privileged",
            action="store_true",
            help="Run act container as privileged (sometimes needed for Docker-in-Docker edge cases).",
        )
        p.add_argument(
            "--verbose",
            action="store_true",
