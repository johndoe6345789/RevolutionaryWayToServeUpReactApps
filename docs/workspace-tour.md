...
- `bootstrap/helpers/`: the logging (`logging.js`), CDN resolver (`network.js`), loader aggregator (`module-loader.js`), tooling helpers (`tools.js`), dynamic icon resolution (`dynamic-modules.js`), source analyzers (`source-utils.js`), and local-module infrastructure (`sass-compiler.js`, `tsx-compiler.js`, `local-paths.js`, `local-module-loader.js`, `local-loader.js`) now publish their APIs on `__rwtraBootstrap.helpers`, letting both the browser bundle (via sequential script tags) and Node-based tests import only what they need.
...
- The new `fallbackProviders` array lets you declare CDN base fallbacks (jsDelivr remains the default); `bootstrap/helpers/network.js:1` will cycle through these whenever `allowJsDelivr` isn't disabled, so you can add private mirrors or additional public CDNs entirely from the config.
- `index.html`, `styles.scss`, and `src/` are the only files copied into the Docker image. `styles.scss` globally sets the dark retro theme, while `src/App.tsx:1`, `src/components/*.tsx`, `src/data.ts:1`, and `src/theme.ts:1` compose the RetroDeck UI, pulling typography, color, and layout tokens out of the theme and reusing shared data constants.
...
- The Docker image described by `Dockerfile:1` uses `node:20-alpine`, installs `http-server`, and copies `index.html`, `bootstrap.js`, `config.json`, `styles.scss`, and `src/` into `/app` so Bun is not required at runtime. `http-server` exposes the bundle on port `4173`, mirroring the port that the smoke test targets.
...
- `python/pyproject.toml:1` defines the `rwtra-scripts` package, which exposes the `bun-wrapper` and `copy-sources` console scripts. `bun-wrapper` (`python/rwtra_scripts/bun_wrapper.py:1`) fetches the latest Bun release, caches it under `$XDG_CACHE_HOME/bun-wrapper`, and forwards CLI arguments to the downloaded binary so developers use a known Bun version without installing it globally. `copy-sources` (`python/rwtra_scripts/copy_sources.py:1`) mirrors select sources (HTML/SCSS/JSON/TSX/TS) into `dist/`, skipping helper directories (`e2e/`, `test-tooling/`, `.git`, etc.) so the generated package only contains the files that need shipping.
