...
- `bootstrap/helpers/`: the logging, CDN resolution, and module tooling helpers are broken into dedicated files so the browser preload sequence can load just the pieces it needs before `bootstrap.js` runs, and CommonJS consumers (tests, scripts) can `require` the functions individually for unit coverage.
