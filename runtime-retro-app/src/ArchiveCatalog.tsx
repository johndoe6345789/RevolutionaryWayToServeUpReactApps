...
interface ArchiveFile { name?: string; size?: string; source?: string; }
...
export default function ArchiveCatalog(): React.JSX.Element {
  ...
  const play = async (item: ArchiveItem): Promise<void> => {
    ...
    if (!system) return;
    setLaunching(item.identifier); setError("");
    try {
      ...
      navigate(`/systems/${platform}?archive=${encodeURIComponent(item.identifier)}&file=${encodeURIComponent(file.name)}`);
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : "Could not inspect this Archive item.");
      setLaunching("");
    }
  };
  ...
  return <section className="archive-catalog">
    ...
    <div className="archive-warning"><strong>Streamed from the source.</strong><span>Games launch client-side from Internet Archive; nothing is copied to this server. Item pages remain available for provenance and rights information.</span></div>
    {loading && <p className="catalog-state">SCANNING INTERNET ARCHIVE…</p>}
    {error && <p className="catalog-state error" role="alert">{error}</p>}
    {!loading && <div className="archive-grid">{items.map((item) => <article key={item.identifier} className="archive-card"><img src={`https://archive.org/services/img/${encodeURIComponent(item.identifier)}`} alt="" loading="lazy" /><div><span>INTERNET ARCHIVE · {item.downloads ?? 0} VIEWS</span><h3>{item.title ?? item.identifier}</h3><p>{text(item.creator)}</p><div className="archive-actions"><button className="button tiny" disabled={launching === item.identifier} onClick={() => void play(item)}>{launching === item.identifier ? "Finding ROM…" : "Play now"}</button><a href={`https://archive.org/details/${encodeURIComponent(item.identifier)}`} target="_blank" rel="noreferrer">Details ↗</a></div></div></article>)}</div>}
  </section>;
}
