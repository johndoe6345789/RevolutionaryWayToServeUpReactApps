export class Chip8 {
  ...
  run(cycles = 24): void {
    for (let count = 0; count < cycles && this.waitingRegister === null; count += 1) this.step();
  }
  ...
  private step(): void {
    ...
    switch (opcode & 0xf000) {
      case 0x0000:
        if (opcode === 0x00e0) { this.display.fill(0); this.changed = true; }
        break;
      case 0x1000: this.programCounter = address; break;
      case 0x3000: if (this.registers[x] === value) this.programCounter += 2; break;
      case 0x4000: if (this.registers[x] !== value) this.programCounter += 2; break;
      case 0x6000: this.registers[x] = value; break;
      case 0x7000: this.registers[x] = (this.registers[x] + value) & 0xff; break;
      case 0xa000: this.index = address; break;
      case 0xc000: this.registers[x] = Math.floor(Math.random() * 256) & value; break;
      case 0xd000: this.draw(this.registers[x], this.registers[y], opcode & 0xf); break;
      case 0xf000:
        if ((opcode & 0xff) === 0x0a) { this.waitingRegister = x; this.programCounter -= 2; }
        break;
    }
  }
