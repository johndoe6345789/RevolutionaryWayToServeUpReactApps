...
function Systems(): React.JSX.Element { return <main className="page section"><span className="kicker">MULTI-SYSTEM DECK</span><div className="page-title"><h1>Play</h1><span>LOCAL OR STREAMED</span></div><p className="lede system-lede">Choose a system to load a local ROM, launch a bundled example, or find something to stream from Internet Archive below.</p><div className="example-shelf"><div><span className="kicker">PLAY RIGHT NOW</span><h2>Bundled examples</h2></div><Link to="/play/pixel-painter" className="example-pill">Pixel Painter <small>CHIP-8</small></Link><Link to="/play/random-stars" className="example-pill">Random Stars <small>CHIP-8</small></Link><Link to="/play/diagonal-drift" className="example-pill">Diagonal Drift <small>CHIP-8</small></Link><Link to="/systems/spectrum?example=csss" className="example-pill featured">CSSS <small>ZX SPECTRUM</small></Link></div><div className="system-grid">{systems.map((system) => <Link key={system.id} to={`/systems/${system.id}`} className={`system-card ${system.colour}`}><span>{system.era}</span><strong>{system.name}</strong><p>{system.description}</p><small>{system.extensions.join(" · ")}</small><i>→</i></Link>)}</div><section className="connected-library"><span className="kicker">CONNECTED COLLECTIONS</span><div className="section-head"><div><h2>Find a game</h2><p>Search and launch software from Internet Archive.</p></div><span>POWERED BY ARCHIVE.ORG</span></div><ArchiveCatalog /></section></main>; }
...
function Catalog(): React.JSX.Element { return <Systems />; }
