  ...
  return (
    <header>
      <div className="nav-wrap">
        ...
        <Link to="/systems" className="button tiny">
          Insert coin
        </Link>
      </div>
    </header>
  ...
  return (
    <>
      <main className="hero">
        ...
        <p className="lede">
          TypeScript fetched, compiled, and executed by your browser.
        ...
        <div className="hero-card">
          ...
          <div className="pixel-grid">
            {Array.from({ length: 96 }, (_, index) => (
              <i
                key={index}
                className={index % 11 === 0 || index % 17 === 0 ? "lit" : ""}
              />
            ))}
          </div>
          <div className="hero-card-label">
            <strong>LIVE RUNTIME</strong>
            <span>TSX → BABEL → REACT</span>
          </div>
        </div>
      </main>
        ...
        <article className={`game-card ${game.accent}`} key={game.id}>
          ...
          <div className="game-body">
            ...
            <div className="card-actions">
              <Link to={`/play/${game.id}`} className="button small">
                Play
              </Link>
              <Link
                to={`/games/${game.id}`}
...
function Library(): React.JSX.Element {
  ...
  return (
    <main className="page section">
      <span className="kicker">PROGRAM DIRECTORY</span>
      <div className="page-title">
        <h1>Game library</h1>
        <span>{filtered.length.toString().padStart(2, "0")} PROGRAMS</span>
      </div>
      <label className="search">
        ...
        <input
          ...
          placeholder="Search titles or systems"
          aria-label="Search games"
        />
      </label>
      <GameGrid items={filtered} />
      {filtered.length === 0 && (
        <p className="empty">NO PROGRAMS MATCH THAT QUERY.</p>
      )}
    </main>
  );
}
...
function GameDetails({ id }: { id: string }): React.JSX.Element {
  ...
  return (
    <main className="page detail">
      ...
      <div className={`detail-art ${game.accent}`}>
        <span>{game.system}</span>
        <strong>{game.title.slice(0, 1)}</strong>
      </div>
      <div className="detail-copy">
        <span className="kicker">ORIGINAL RUNTIME ROM · {game.year}</span>
        <h1>{game.title}</h1>
        ...
        <dl>
          ...
          <div>
            <dt>Display</dt>
            <dd>64 × 32 monochrome</dd>
          </div>
          <div>
            <dt>Input</dt>
            <dd>16-key hexadecimal pad</dd>
          </div>
        </dl>
        <Link to={`/play/${game.id}`} className="button">
          Run program →
        </Link>
      </div>
    </main>
  ...
  return (
    <main className="page play-page">
      <div className="play-heading">
        <div>
          <span className="kicker">NOW EMULATING</span>
          <h1>{game.title}</h1>
        </div>
...
function Systems(): React.JSX.Element {
  return (
    <main className="page section">
...
function SystemPlayer({ id }: { id: string }): React.JSX.Element {
  const system = findSystem(id);
  if (!system) return <NotFound />;
  return (
    <main className="page system-page">
      <Link to="/systems" className="text-link">
        ← All systems
      </Link>
      <div className="play-heading">
        <div>
          <span className="kicker">{system.era} HARDWARE PROFILE</span>
          <h1>{system.name}</h1>
        </div>
        <span className="local-badge">ROM NEVER UPLOADED</span>
      </div>
      <ConsoleArcade system={system} />
      <p className="legal-copy">
        Only load homebrew, public-domain software, or cartridge/tape dumps you
        are legally entitled to use. Runtime Arcade does not provide commercial
        ROMs.
      </p>
    </main>
  );
...
function Technology(): React.JSX.Element {
  return (
    <main className="page section prose">
      <span className="kicker">UNDER THE CARTRIDGE</span>
      <h1>TypeScript with no build step.</h1>
      <p className="lede">
        The source files powering this page are still TypeScript and TSX when
        the server sends them. The bootloader turns them into executable modules
      ...
      <div className="flow">
        <div>
          <b>01</b>
          <strong>Fetch</strong>
          <span>The loader requests App.tsx and discovers its imports.</span>
        </div>
        <i>→</i>
        <div>
          <b>02</b>
          <strong>Compile</strong>
          <span>Babel Standalone strips types and transforms JSX.</span>
        </div>
        <i>→</i>
        <div>
          <b>03</b>
          ...
          <span>
            A module registry resolves dependencies and React renders.
          </span>
        </div>
        <i>→</i>
        <div>
          <b>04</b>
          <strong>Route</strong>
          <span>History API navigation swaps views without a reload.</span>
        </div>
      </div>
      <pre>
        <code>{`// This component reached you as TSX\ninterface GreetingProps { name: string }\nexport function Greeting({ name }: GreetingProps) {\n  return <h1>Hello, {name}</h1>;\n}`}</code>
      </pre>
      <div className="proof">
        <span id="compile-proof">●</span>
        <div>
          <strong>Runtime proof is active</strong>
          <small>
            Inspect window.__RUNTIME_RETRO__ for compiler timing and module
            count.
          </small>
        </div>
      </div>
...
function About(): React.JSX.Element {
  return (
    <main className="page section prose">
      ...
      <h1>Make the browser feel magical again.</h1>
      <p className="lede">
        Runtime Arcade revives an ambitious idea: source code should be
        immediately playable. It pairs a custom zero-build React bootloader with
        a tiny, honest emulator and original programs that are safe to share.
      </p>
      <div className="manifesto">
        <h2>Why this matters</h2>
        <p>
          Most React deployments hide the source behind a build pipeline. This
          experiment asks what happens when the browser receives human-readable
          TypeScript, resolves the module graph, compiles it, and runs it
          directly. The result is slower to boot than a bundled production
          app—but radically transparent, portable, and fun.
        </p>
      </div>
...
function NotFound(): React.JSX.Element {
  return (
    <main className="page not-found">
      ...
      <p>That route is not registered in this runtime.</p>
...
function RoutedApp(): React.JSX.Element {
  ...
  return (
    <div className="site">
      ...
      <footer>
        <Logo />
        <p>Source code delivered raw. Compiled with curiosity.</p>
        <div>
          <Link to="/systems">Play</Link>
